...
- Reworked the Kaocha CLI surface so `scry.cli --runner kaocha` is a drop-in for Kaocha's own command line. **Breaking (`-m`):** scry now consumes only its own flags (`--runner`/`-r`, `--help`/`-h`, `--result-format`, `--config`, `--dir`/`-d`) and forwards every other `-m` argument — Kaocha options (e.g. `--focus my.ns/test-foo`, `--no-randomize`) and positional `[SUITE]...` selectors — verbatim to Kaocha's own CLI parser. The scry-specific `-m --focus SYM` and `-m --kaocha-opt KEY VALUE` flags are removed (use plain forwarding), as is the `--suite`/`-s`/`--suites` flag set; suite selectors remain trailing positionals. Forwarded cli-options merge into the resolved config's `:kaocha/cli-options` (an explicit `:config` wins) and forwarded positionals use the existing exact-id-then-unique-text suite resolution. A malformed Kaocha option on `-m` now surfaces as a Kaocha runner/load error instead of an argument error; core-only `--namespace`/`--var`/`--ns-pattern` selectors stay rejected with a clean argument error in Kaocha mode. The `-X` `:suite`/`:suites`/`:kaocha-extra` keys, `-X` mistyped-key pass-through, and the `scry.kaocha/run` adapter API are unchanged (the adapter gains a documented `-m`-only `:kaocha-argv` option), and core (`--runner clojure-test`) mode is unaffected.
