   ...
   :overrides {'scry.cli
               {'run {:meta {:arglists '([opts])}
                      :doc (str "`clojure -X` entry point for the scry CLI.\n\n"
                                ...
                                "Completed concrete failure/error entries are synchronously published "
                                "as atomic final `.edn` artifacts before the next test var begins; "
                                "synthetic entries and missed publications are reconciled after a normal "
                                "runner return. A catchable runner error preserves already published "
                                "final paths in `:result-files`.\n\n"
                                "Contained artifact-publication failures preserve the test-derived outcome "
                                "and add bounded top-level `:scry.cli/diagnostic-error` metadata only "
                                "while an artifact remains unresolved. Its `:phase` is "
                                ...
                                "`:root-message`, and `:failed-entry-count`; when derivable it also "
                                "includes `:first-failing-var` and `:first-root-cause`.\n\n"
