...
- Use `bb/scry/api_docs.clj` (`scry.api-docs`) as the single source-controlled generator entry point. Add a docs-only `:quickdoc` alias that puts `bb/` and the pinned quickdoc dependency on the generator classpath, and have both `bb api-docs` and `bb api-docs --check` shell through the same Clojure entry point: `clojure -M:quickdoc:kaocha -m scry.api-docs` with `--check` forwarded for no-diff verification. The `:kaocha` alias supplies `src-kaocha` and the optional Kaocha dependency; the base `:paths` supplies `src`.
...
- Drive all generated-only prose from source-controlled generator inputs in `bb/scry/api_docs.clj`: pre-1.0/public-alpha status, the optional Kaocha classpath note, the regeneration/check commands, the relationship to README workflow docs, and CLI examples using README-aligned aliases.
...
6. **Verification and task notes** — run `bb api-docs --check`, `git diff --check`, formatting/linting for touched Clojure/build/task files, focused load/tests if runtime-adjacent files changed, and record decisions/results in `implementation.md`. Include the concrete dependency-boundary verification `clojure -M:test:build -e "(require '[scry.build-test :as t] '[clojure.test :as ct]) (let [r (ct/run-tests 'scry.build-test)] (when-not (ct/successful? r) (System/exit 1)))"`; these focused build/POM checks prove published core and adapter POM dependencies remain curated and that the core artifact excludes the optional Kaocha adapter. Also inspect `deps.edn`/diff to confirm `borkdude/quickdoc` appears only under the docs-only `:quickdoc` alias, not top-level `:deps`.
