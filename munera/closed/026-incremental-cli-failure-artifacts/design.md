...
- diagnostic ordering is deterministic: after a normal return, unresolved concrete paths represented in the canonical vector follow first occurrence in canonical result order, callback-only concrete paths follow in completion order, and synthetic paths then follow their assignment order; a no-reconciliation runner-error uses completion order; `:first-failing-var` and `:first-root-cause` describe the first unresolved item in that order when derivable;
