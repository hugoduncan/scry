...
- [ ] Add focused diagnostic-write-failure coverage for a `:scry.cli/zero-tests` outcome, proving the fallback preserves non-zero exit/`:scry.cli/zero-tests`, attaches bounded `:scry.cli/diagnostic-error`, leaves result files empty, and does not emit failure-details pointer text.
- [ ] Add a direct sanitizer test for Throwable `ex-data` depth limiting using a configured small `:max-ex-data-depth`, asserting nested ex-data is truncated independently of the outer data depth.
