...
- [ ] Add focused diagnostic-write-failure coverage for a passing `:scry.cli/pass` outcome, proving post-run diagnostic write failure preserves exit code `0`/`:scry.cli/pass` while attaching bounded `:scry.cli/diagnostic-error` and empty result files.
