...
- [ ] Move the map-shaped assertion actual regression in `test/scry/cli_test.clj` inside a `deftest`; it is currently a top-level `(testing ...)` form, so its assertions run at namespace load time rather than as an isolated clojure.test test.
