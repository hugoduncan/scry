...
- [x] Add a true persistent-set truncation sentinel collision regression for `edn-readable-data`; the current strengthened collision case uses `java.util.LinkedHashSet`, which exercises the generic `Iterable` branch rather than the Clojure `set?` branch, so the set-specific sanitizer behavior remains under-specified.
- [x] Add direct sanitizer regressions for hostile collection boundaries (for example Java `Map`/`Iterable` values whose iteration, entry access, or size/probing throws) so pathological data graphs are proven to sanitize or fall back boundedly rather than escaping as unstructured diagnostic-write failures.
- [x] Add an exec-level (`clojure -X` or `run-with-boundary`) pathological fixture regression that exercises the real clojure-test runner and asserts the structured non-zero ex-data/outcome preserves `:scry.cli/test-failure` with bounded result files/diagnostics, rather than relying on the `-m` subprocess test's synthetic in-test outcome map.
- [x] Add direct sanitizer regressions for hostile `Throwable` accessor boundaries (for example `getMessage`, `getCause`, `getStackTrace`, `getSuppressed`, or `ex-data` access throwing) so controlled Throwable normalization is proven bounded/placeholder-producing rather than escaping to diagnostic-write fallback for pathological Throwable objects.
- [x] Add a one-shot or side-effecting `Iterable` sanitizer regression proving max-seq-length truncation detection does not depend on traversing an iterable twice; either preserve the retained values plus explicit truncation sentinel deterministically or fall back to a bounded placeholder.
- [x] Add focused diagnostic-write-failure coverage for a first failing/erroring assertion whose `:actual` is a hostile `Throwable` with throwing `getCause` and/or `getMessage`, and make `root-cause-throwable` / `throwable-cause-text` tolerate those accessors without converting the post-run diagnostic failure into `:scry.cli/runner-error`.
...
- [x] Document the stable public shape of top-level `:scry.cli/diagnostic-error` metadata in README.md and the generated `scry.cli/run` API docs, including the required keys users can inspect (`:phase`, `:message`, `:type`, `:root-type`, `:root-message`, `:failed-entry-count`, plus optional/when-derivable `:first-failing-var` and `:first-root-cause`).
