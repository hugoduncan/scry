...
- [ ] Add focused diagnostic-write-failure coverage for `:scry.cli/unknown-result`, proving the fallback preserves the unknown-result outcome, attaches bounded `:scry.cli/diagnostic-error`, leaves result files empty, and still emits the existing unknown-result stderr pointer semantics.
