...
- [ ] Add a true persistent-set truncation sentinel collision regression for `edn-readable-data`; the current strengthened collision case uses `java.util.LinkedHashSet`, which exercises the generic `Iterable` branch rather than the Clojure `set?` branch, so the set-specific sanitizer behavior remains under-specified.
