...
- [ ] Add direct sanitizer regressions for hostile collection boundaries (for example Java `Map`/`Iterable` values whose iteration, entry access, or size/probing throws) so pathological data graphs are proven to sanitize or fall back boundedly rather than escaping as unstructured diagnostic-write failures.
- [ ] Add an exec-level (`clojure -X` or `run-with-boundary`) pathological fixture regression that exercises the real clojure-test runner and asserts the structured non-zero ex-data/outcome preserves `:scry.cli/test-failure` with bounded result files/diagnostics, rather than relying on the `-m` subprocess test's synthetic in-test outcome map.
