...
- [ ] Make CLI diagnostic fallback root-cause traversal cycle-safe/bounded (`root-cause-throwable`, `throwable-cause-text`, and `assertion-cause-text`) so a diagnostic-write failure cannot hang or overflow when the serialization exception or first failing Throwable has a cyclic/deep cause chain.
- [ ] Bound all string fields in top-level `:scry.cli/diagnostic-error` and the matching stderr fallback (`:message`, `:root-message`, and `:first-root-cause`) so hostile/large exception messages or assertion actuals cannot produce unbounded structured outcomes or fallback output.
