...
- [ ] Document the stable public shape of top-level `:scry.cli/diagnostic-error` metadata in README.md and the generated `scry.cli/run` API docs, including the required keys users can inspect (`:phase`, `:message`, `:type`, `:root-type`, `:root-message`, `:failed-entry-count`, plus optional/when-derivable `:first-failing-var` and `:first-root-cause`).
