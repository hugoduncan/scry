...
- [ ] Add direct sanitizer tests proving `:max-seq-length` truncation emits an explicit bounded/truncation sentinel for each supported collection shape (sequential, vector, set, map/Java Map, array, Iterable), not only silently dropping excess values.
- [ ] Add direct Throwable normalization tests that assert the documented frame field shape for `:at` and `:trace` entries, so result-file EDN locks the intended bounded frame representation rather than an accidental map/vector mismatch.
