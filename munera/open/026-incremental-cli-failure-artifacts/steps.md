...
- [ ] Apply Kaocha's forwarded `--plugin` / `:kaocha-extra :plugin` selections to `:kaocha/plugins` before `ensure-runtime-plugins` appends the completion observer. Currently `apply-kaocha-extra` only stores `:plugin` under `:kaocha/cli-options`, so `api/run` never loads the requested plugin and the observer cannot run after its `post-test` hook as required. Add `-m` and `-X` regressions proving the selected plugin executes and its leaf mutation is visible in the completion callback/artifact while `:scry.kaocha/completed-entry` remains last.
